...
void button_pressed(){
    ...
    char shutdown_message[50];
    snprintf(shutdown_message, 50, "%02d:%02d:%02d SHUTDOWN\n", info->tm_hour, info->tm_min, info->tm_sec);
    int sslwrite_ret = SSL_write(ssl_client, shutdown_message, strlen(shutdown_message));
    if (sslwrite_ret <= 0){
        SSL_shutdown(ssl_client);
        SSL_free(ssl_client);
        print_error("Error writing ID message to socket via SSL", 2, 0);
    }
    if (log_flag == 1)
...
int main(int argc, char** argv){
    ...
    if (sockfd < 0)
        print_error("Error trying to open socket", 2, 1);
    ...
    if (connect_retval < 0)
        print_error("Error while connecting to server", 2, 1);
    ...
    if (new_context == NULL)
        print_error("Unable to get SSL context", 2, 0);
    
    ssl_client = SSL_new(new_context);
    if (ssl_client == NULL)
        print_error("Unable to complete SSL setup", 2, 0);
    
    int ssl_retval;
    ssl_retval = SSL_set_fd(ssl_client, sockfd);
    if (ssl_retval == 0)
        print_error("Assocating TCP socket to SSL client with SSL_set_fd() failed", 2, 0);

    ssl_retval = SSL_connect(ssl_client);
    if (ssl_retval < 1)
        print_error("Making actual SSL connection with SSL_connect() failed", 2, 0);


    char id_message[25];
    snprintf(id_message, 25, "ID=%d\n", id);
    int sslwrite_ret = SSL_write(ssl_client, id_message, strlen(id_message));
    if (sslwrite_ret <= 0){
        SSL_shutdown(ssl_client);
        SSL_free(ssl_client);
        print_error("Error writing ID message to socket via SSL", 2, 0);
    ...
    if (temperature_sensor == NULL){
        mraa_deinit();
        print_error("Initializing aio for temperature sensor failed.", 2, 0);
    }
    ...
    while (button_press == 0){
        ...
        if ((stop_reporting == 0) && (current_time.tv_sec >= next_temp_time)){
            ...
            char tt_report[50];
            snprintf(tt_report, 50, "%02d:%02d:%02d %.1f\n", info->tm_hour, info->tm_min, info->tm_sec, T);
            sslwrite_ret = SSL_write(ssl_client, tt_report, strlen(tt_report));
            if (sslwrite_ret <= 0){
                SSL_shutdown(ssl_client);
                SSL_free(ssl_client);
                print_error("Error writing the temp and time report to socket via SSL", 2, 0);
            }
            if (log_flag == 1){
        ...
        if (poll_retval < 0)
            print_error("poll system call failed", 2 , 1);
        ...
        if (poll_retval != 0){
            if (fds[0].revents & POLLIN){
                ...
                int bytes_read = SSL_read(ssl_client, (void *)read_buffer, 100);
                if (bytes_read <= 0){
                    SSL_shutdown(ssl_client);
                    SSL_free(ssl_client);
                    print_error("Error trying to read input from socket via SSL", 2, 0);
                }
    ...
    if (status != MRAA_SUCCESS){
        SSL_shutdown(ssl_client);
        SSL_free(ssl_client);
        print_error("Error closing aio temperature sensor", 2, 0);
    }
