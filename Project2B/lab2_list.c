...
int main(int argc, char** argv){
    ...
    while(1){
        ...
        switch(c){
            ...
            default: //? is returned because an incorrect flag was used
                fprintf(stderr, "Incorrect flag usage. Allowed flags: --yield=[idl] --lists=# --sync=[ms] --threads=# --iterations=#\n");
                exit(1);
            }
    }
    ...
    //create array of ints for spin lock and initialize to 0 using calloc
    else if (spin_flag == 1)
    {
    ...
    //check length of each list at end
    ...
    //if mutex option, free all created mutex's
    if (mutex_flag == 1){
        int destroy_ret;
        for (int i = 0; i < num_list; i++){
    ...
    //if spin option, free int array used for locks
    else if (spin_flag == 1){
        free(spin_lock_array);
    }
    //free list heads, all elements, and each key
